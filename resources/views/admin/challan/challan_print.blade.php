...
<div class="footer">
    ...
    <span style="text-align: left !important;font-weight: 600">Delivery Challan Prepared By : {{$quot->first_name}} {{$quot->last_name}}</span>
...
<table style="border: none">
    <tr>
        <td style="width:30%;vertical-align: top;border: none;text-align: left">
            <!-- -----left column----- -->
        ...
        <td style="width: 35%;vertical-align: top;border: none;padding: 15px">
            ...
            <table align="center" style="width: 100%;padding-top: 10px">
                ...
                <tr>
                    <td style="text-align: left"><b>Phone :</b>{{$customer->primary_phone}}</td>
                </tr>
                <tr>
                    <td style="text-align: left"><b>Email :</b> {{$customer->primary_email}}</td>
                </tr>
            </table>
        ...
        <td style="width: 35%;vertical-align: top;border: none;padding: 15px">
            ...
            <table align="center" style="width: 100%">
                <tr>
                    <th colspan="2">Delivery Challan No. : {{$quot->challan_number}}</th>
                </tr>
                <tr>
                    <td style="text-align: left;border-right: none;width: 40%"><b>Delivery Challan Date</b> </td>
                    <td style="border-left: none;text-align: left;width: 60%"> : {{date('d-m-Y',strtotime($quot->invoice_date))}}</td>
                </tr>
                <tr>
                    <td style="text-align: left;border-right: none"><b>Valid Date</b> </td>
                    <td style="border-left: none;text-align: left"> : {{date('d-m-Y',strtotime($quot->invoice_duedate))}}</td>
                </tr>
            </table>
...
<table style="border-collapse: collapse;padding-top: 10px;width: 100%">
    ...
    @foreach($quotitem as $item)
        ...
        <tr>
            ...
            <td style="text-align: left;vertical-align:top;">{{$item->product_name}}
                <br><span style="font-size: 8px">{{$item->description}}</span>
            </td>
            {{--            <td style="text-align: center;vertical-align:top;width: 3%">{{$item->inner_daimitter}}</td>--}}
            {{--            <td style="text-align: center;vertical-align:top;width: 3%">{{$item->outer_daimitter}}</td>--}}
            {{--            <td style="text-align: center;vertical-align:top;width: 3%">{{$item->thk}}</td>--}}
            ...
            <td style="text-align: right;vertical-align:top;width: 3%">{{number_format($item->total_amount,2,'.',',')}}</td>
            ...
            @if($item->igst_per==0)
                ...
                {{--                <td style="text-align: right;vertical-align:top;width: 3%">{{number_format($gst_total,2,'.',',')}}</td>--}}
            @else
                <?php
                $gsttotal =$gsttotal+ $item->gst_amount;
                ?>
                <td style="text-align: right;vertical-align:top;width: 3%">{{$item->igst_per}}</td>
                {{--                <td style="text-align: right;vertical-align:top;width: 3%">{{$item->gst_amount}}</td>--}}
            @endif

            <td style="text-align: right;vertical-align:top;width: 3%">{{number_format($item->net_price,2,'.',',')}}</td>
    ...
    <tr>
        ...
        <td>{{number_format($quot->item_total,2,'.',',')}}</td>
        <td></td>
        <td>{{number_format($quot->discount_total,2,'.',',')}}</td>
    ...
    ?>
    <tr>
        <td style="text-align: left" colspan="10">Adjustment Off</td>
        <?php
        $round=round($grand);
        $diff=$round-$grand;
        $gtot=$grand+$diff;
    ...
    <tr>
        ...
        <td style="text-align: right;font-weight: 800">{{number_format($quot->grand_total,2,'.',',')}}</td>
    </tr>
    <tr class="total" style="  background-color: #ccc;">
        <td style="text-align: left;font-weight: 800;padding-top: 12px;padding-bottom: 12px" colspan="11">
            <b>In Words :-
                ...
                echo $word = $f->format($quot->grand_total);
                ?>
            </b>
        </td>
    </tr>
