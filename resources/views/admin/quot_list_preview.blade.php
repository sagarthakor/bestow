  ...
  <div class="row">
    ...
    <div class="col-sm-12">
     ...
     <?php $srno++; ?>
     <tr>
      <td style="width: 5%;vertical-align: top;"> {{($list->currentPage() - 1) * $list->perPage() + $loop->iteration}}</td>
      <td width="12%"  style="vertical-align: top;">
        <a href="{{url('client/quot/view/'.$data->id)}}" target="_blank">{{$data->quotation_no}}</a></td>
        ...
        <td width="10%"  style="vertical-align: top;">
          {{date('d-m-Y',strtotime($data->quot_date))}}
                           ...
                           <!--      <td  style="vertical-align: top;"></td>
                            ...
                            <td  style="vertical-align: top;width:7%;text-align: left;">{{number_format($data->grand_total)}}
                            </td>
                            <td  style="vertical-align: top;text-align: center;width: 10%">{{$data->quot_stage}}</td>

                            <td  style="vertical-align: top;width: 15%">
                                @can('quotation_update')
                                    <a class="table-btn" title="edit"  href="{{url('admin/quotation_edit/'.$data->id)}}" title="edit" target="_blank"><i class="fa fa-pencil" style="margin:10px;font-size: 10px;"></i></a>
                                    <a class="table-btn" title="preview" target="_blank" href="{{url('admin/quot_preview/'.$data->id)}}" target="_blank"><i class="fa fa-eye" style="margin:10px;font-size: 10px;"></i></a>
                                @endcan
                              ...
                              @can('quotation_print')
                                        <a class="table-btn" title="pdf" target="_blank" href="{{url('admin/quot_print/'.$data->id)}}"><i class="fa fa-file-pdf-o" style="margin:10px;font-size: 10px;"></i></a>
                              @endcan
                             ...
                             <!--  <a class="table-btn modalopen" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal"  data-id="{{$srno}}" title="mail" href="#">
                              ...
                              @if($data->so_status=="Y")
                              @else
                              <a style="display: none" class="table-btn" title="sales order" href="{{url('client/sales_order/create/'.$data->id)}}" ><button class="btn btn-primary btn-sm">Create SO.</button></a>
                              @endif
