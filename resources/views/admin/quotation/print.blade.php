    ...
    @foreach($quotitem as $item)
        ...
        <tr>
            ...
            <td style="text-align: left;vertical-align:top;">{{$item->product_name}}
               <textarea style="white-space: pre-wrap">{{$item->description}}</textarea>
...
{{--            </td>--}}
    ...
    <tr>
        <td style="text-align: left" colspan="11">Discount</td>

        <td style="text-align: right">{{number_format($discount,2,'.',',')}}</td>
    </tr>
    @if($totcgst !="" || $totcgst != 0)
        <tr>
            <td style="text-align: left" colspan="11">CGST</td>

            <td style="text-align: right">{{number_format($totcgst,2,'.',',')}}</td>
        </tr>

        <tr>
            <td style="text-align: left" colspan="11">SGST</td>

            <td style="text-align: right">{{number_format($totsgst,2,'.',',')}}</td>
        </tr>
    @else
        <tr>
            <td style="text-align: left" colspan="11">IGST</td>
    ...
    ?>
    <tr>
        <td style="text-align: left" colspan="11">Rounf Off</td>
        <?php
        $round=round($grand);
        $diff=$round-$grand;
        $gtot=$grand+$diff;
    ...
    <tr>
        <td style="text-align: left;font-weight: 800" colspan="11">Grand Total</td>

        <td style="text-align: right;font-weight: 800">{{number_format($round,2,'.',',')}}</td>
    </tr>
    <tr class="total" style="  background-color: #ccc;">
        <td style="text-align: left;font-weight: 800;padding-top: 12px;padding-bottom: 12px" colspan="12">
            <b>In Words :-
                <?php
    ...
    <tr>
        <th colspan="12" style="text-align: left;">Terms & Conditions</th>
    </tr>
    <tr>
