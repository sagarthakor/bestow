                                ...
                                <div style="display:inline-block;margin-right:5px;margin-top: 5px;width: 100%">
                                    <div class="col-md-6">
                                        ...
                                        <select name="customer_name" id="customer_name"  class="submit_on_enter customer_name js-example-basic-single">
                                            ...
                                            @foreach($vendor as $cu)
                                                <option value="{{$cu->id}}">{{$cu->vendor_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
    ...
    // Hide image container
      ...
      function delete_item(id)
      {
        $("#myModal").modal('hide');
          var appurl="{{ url('/') }}";
          $.ajax({
            url:appurl+'/po_cart_item_remove',
            data:{id:id},
            method:'get',
            beforeSend: function(){
    // Show image container
      ...
      function checkout()
      {
        ...
        if(customer_name=="")
        {
            ...
            var appurl = "{{url('/')}}";
            window.location = appurl + "/checkout/po/" + customer_name;
        }
